...
#        pdb.set_trace()
    ...
    def make_gridmask(self, with_pad=True, use_maskoceans=False):   #False
        ...
        self.M = Basemap(projection='gall', llcrnrlon = self.lonmin,                                             urcrnrlon = self.lonmax,                                             llcrnrlat = self.latmin,                                             urcrnrlat = self.latmax,                                             lat_ts = 0.5 * (self.latmin + self.latmax),                                             resolution = 'h') # 'h'-high, 'l'-low  
...
if __name__ == '__main__':
    ...
    inputdirectory='E:/eddy/3/'
    eddy_select_info['outfile']='E:/eddy/'  #
    ...
    lon1,lat1=-179.875,-90#30.125,-90
...
#    
    ...
    for k in range (len(AVISO_files)): #
        ...
        #pdb.set_trace() 
        udata = AVISO_data.variables['ugosa'][0]#获得u值
        udata=np.hstack((udata,udata[:,0:exstra_range])) 
        vdata = AVISO_data.variables['vgosa'][0]#获得v值
